...
import { IPaginationParams, IPlaylist, ITrack } from "@shared/types";
import { IManagePlaylistTracksPayload, PlaylistService } from "../api";
import { PlaylistActionTypes, PlaylistMode } from "./types";

export const addToCurrentPlaylist = createAction<ITrack>(PlaylistActionTypes.ADD_TO_CURRENT_PLAYLIST);
export const resetModalState = createAction(PlaylistActionTypes.RESET_MODAL_STATE);
...
export const toggleCheckbox = createAction<string>(PlaylistActionTypes.TOGGLE_CHECKBOX);
...
export const loadState = createAsyncThunk(PlaylistActionTypes.LOAD_PLAYLIST_STATE, async (loadingState: IPlaylist, ta) => {
  try {
