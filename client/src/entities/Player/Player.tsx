...
import { audio, setAudioInstance, generateUrl } from "@shared/lib";
import { usePlayerActions, usePlayerSelector, usePlayerControl } from "./lib";

export const Player: React.FC = () => {
  const { active, currentTime, duration, pause, volume } = usePlayerSelector();
  const dispatch = useAppDispatch();
  const { playControl, nextTrack, prevTrack, playTrack } = usePlayerControl();
  const { setCurrentTime, setDuration, setVolume, startNext } = usePlayerActions();
